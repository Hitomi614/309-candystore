...
class Auto extends CI_Controller {
	...
	// checks that email is of form [something]@[something].[something]
	...
	public function ccard_check($ccard) {
		if (preg_match("/^\d{16}$/", $ccard) == 0) {
			$this->form_validation->set_message('ccard_check', 'Invalid credit card number.');
			return false;
		}
		return true;
	}
	...
	public function ccard_date_check($ccard_date) {
		if (preg_match("/^(0[1-9]|1[0-2])\/[0-9]{2}$/", $ccard_date) == 0) {
			$this->form_validation->set_message('ccard_date_check', 'Invalid date format.');
			return false;
		}
		return true;
	}
